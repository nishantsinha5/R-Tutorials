...
docache <- T  ## set FALSE to rerun GAM smoothings
```
...
# A Default ggplot
`ggplot2` syntax is different from base R. We always start to define a plotting element by calling `ggplot(data, aes(variable1, variable1))` which just tells `ggplot2` that we are going to work with that data. Thus, only a panel is created if we run this since `ggplot2` does not now **how** we want to plot that data.
```{r ggplot}
...
```{r extrafont package, warning = F, message = F, cache = F}
library(extrafont)
font_import()
...
```{r plots combined 1, warning = F, cache = F}
...
```{r plots combined 2, warning = F, cache = F}
p1 <- ggplot(chic, aes(date, temp, color = factor(season))) + 
...
```{r ggthemes economist, warning = F, cache = F}
library(ggthemes)
...
```{r scale_color_tableau, message = F, cache = F}
library(ggthemes)
...
```{r viridis continuous, message = F, warning = F, cache = F}
library(viridis)
...
```{r data text, warning = F, message = F, cache = F}
set.seed(2017)
library(tidyverse)
...
```{r melt correlation matrix, warning = F, cache = F}
library(reshape2)
corm <- melt(corm)
