...
options(width = 700)
knitr::opts_chunk$set(cache = F, out.width = "100%")
...
```{r library, message = F, warning = F, cache = F}
library(ggplot2)
```
...
```{r plots combined 2, warning = F, message = F, cache = F}
p1 <- ggplot(chic, aes(date, temp, color = factor(season))) + 
...
```{r scale_color_tableau, warning = F, message = F, cache = F}
library(ggthemes)
...
```{r melt correlation matrix, warning = F, message = F, cache = F}
library(reshape2)
corm <- melt(corm)
